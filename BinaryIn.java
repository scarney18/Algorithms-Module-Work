 ...
 * binary input stream, either one bit at a time (as a {@code boolean}), 8 bits
 * at a time (as a {@code byte} or {@code char}), 16 bits at a time (as a
 * {@code short}), 32 bits at a time (as an {@code int} or {@code float}), or 64
 * bits at a time (as a {@code double} or {@code long}).
 * <p>
 ...
 * <p>
 * The client should not intermix calls to {@code BinaryIn} with calls to
 * {@code In}; otherwise unexpected behavior will result.
 *
...
public final class BinaryIn {
	 ...
	 *
	 * @return {@code true} if this binary input stream exists; {@code false}
	 *         otherwise
	 */
	 ...
	 *
	 * @return {@code true} if this binary input stream is empty; {@code false}
	 *         otherwise
	 */
	...
	/**
	 * Reads the next 8 bits from this binary input stream and return as an 8-bit
	 * char.
	 *
	...
	/**
	 * Reads the next r bits from this binary input stream and return as an r-bit
	 * character.
	 *
	 * @param r number of bits to read
	 * @return the next {@code r} bits of data from this binary input streamt as a
	 *         {@code char}
	 * @throws NoSuchElementException   if there are fewer than {@code r} bits
	 *                                  available
	...
	/**
	 * Reads the remaining bytes of data from this binary input stream and return as
	 * a string.
	 *
	 * @return the remaining bytes of data from this binary input stream as a
	 *         {@code String}
	 * @throws NoSuchElementException if this binary input stream is empty or if the
	 *                                number of bits available is not a multiple of
	 *                                8 (byte-aligned)
	 */
	...
	/**
	 * Reads the next 16 bits from this binary input stream and return as a 16-bit
	 * short.
	 *
	 * @return the next 16 bits of data from this binary input stream as a
	 *         {@code short}
	...
	/**
	 * Reads the next 32 bits from this binary input stream and return as a 32-bit
	 * int.
	 *
	 ...
	 *
	 * @param r number of bits to read
	 * @return the next {@code r} bits of data from this binary input stream as a
	 *         {@code int}
	...
	/**
	 * Reads the next 64 bits from this binary input stream and return as a 64-bit
	 * long.
	 *
	...
	/**
	 * Unit tests the {@code BinaryIn} data type. Reads the name of a file or URL
	 * (first command-line argument) and writes it to a file (second command-line
	 * argument).
	 *
